...
$(document).ready(function() {
    var socket = new WebSocket('ws://http://webprocess-ksanork.rhcloud.com:8000'),
        gethosts = {
            'type': 'get-hosts',
            'content': null
        };
            ...
            switch(json.type) {
                case "get-hosts-result":
                    for(var i = 0; i < json.content.length; i++) {
                        var enabledclass = '';
                        if(!json.content[i].connected) enabledclass = 'disabled';
                        
                        console.log(json.content[i]);
                        
                        var div = "<div host-id=\"" + json.content[i]._id + "\" class=\"host " + enabledclass + "\">" +
